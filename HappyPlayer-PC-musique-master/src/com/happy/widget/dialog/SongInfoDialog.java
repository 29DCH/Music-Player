...
public class SongInfoDialog extends JDialog {
	...
	singerIcon.setImage(singerIcon.getImage().getScaledInstance(iconSize, iconSize, Image.SCALE_SMOOTH));
	iconJLabel = new JLabel(singerIcon);

	iconJLabel.setBounds(padding, padding, iconSize, iconSize);

	//
	displayNameJLabel = new JLabel("歌名");
	displayNameJLabel.setBounds(iconJLabel.getX() + iconJLabel.getWidth() + padding, padding,
		width - iconJLabel.getX() - iconJLabel.getWidth() - padding, componentHeight);

	JLabel sizeTextJLabel = new JLabel("大小:");
	sizeTextJLabel.setBounds(iconJLabel.getX() + iconJLabel.getWidth() + padding,
		displayNameJLabel.getY() + displayNameJLabel.getHeight() - padding, componentWidth - padding * 2,
		componentHeight);

	//
	sizeJLabel = new JLabel("0.00MB");
	sizeJLabel.setBounds(sizeTextJLabel.getX() + sizeTextJLabel.getWidth(),
		displayNameJLabel.getY() + displayNameJLabel.getHeight() - padding, componentWidth + padding,
		componentHeight);
	...
	fileTypeJLabel.setBounds(fileTypeTextJLabel.getX() + fileTypeTextJLabel.getWidth(),
		displayNameJLabel.getY() + displayNameJLabel.getHeight() - padding, componentWidth, componentHeight);
	//
	JLabel timbreTextJLabel = new JLabel("音质:");
	timbreTextJLabel.setBounds(iconJLabel.getX() + iconJLabel.getWidth() + padding,
		sizeTextJLabel.getY() + sizeTextJLabel.getHeight() - padding, componentWidth - padding * 2,
		componentHeight);
	timbreJLabel = new JLabel("普通");
	timbreJLabel.setBounds(timbreTextJLabel.getX() + timbreTextJLabel.getWidth() + padding,
		sizeTextJLabel.getY() + sizeTextJLabel.getHeight() - padding, componentWidth, componentHeight);

	//
	JLabel sourceTextJLabel = new JLabel("来源:");
	sourceTextJLabel.setBounds(timbreJLabel.getX() + timbreJLabel.getWidth(),
		sizeTextJLabel.getY() + sizeTextJLabel.getHeight() - padding, componentWidth - padding * 2,
		componentHeight);

	sourceJLabel = new JLabel("本地");
	sourceJLabel.setBounds(sourceTextJLabel.getX() + sourceTextJLabel.getWidth(),
		sizeTextJLabel.getY() + sizeTextJLabel.getHeight() - padding, componentWidth, componentHeight);
