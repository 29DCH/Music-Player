...
public class PlayListPanel extends JPanel implements Observer {
	...
	jScrollPane.getViewport().setBackground(new Color(255, 255, 255, Constants.listViewAlpha));
	...
	for (int i = 0; i < mCategorys.size(); i++) {
	    ...
	    ListViewItemHeadPanel listViewItemHeadPanel = new ListViewItemHeadPanel(this, listViewPanel, width,
		    titleName, size, listViewItemComPanel, mSongInfo, i);
	    for (int j = 0; j < mSongInfo.size(); j++) {
		SongInfo songInfo = mSongInfo.get(j);
		refreshListViewItemComPanelUI(i, j, listViewItemComPanel, songInfo);
    ...
    private void refreshListViewItemComPanelUI(int pindex, int sindex, ListViewItemComPanel listViewItemComPanel,
	    SongInfo songInfo) {
	ListViewItemComItemPanel listViewItemComItemPanel = new ListViewItemComItemPanel(this, listViewPanel, pindex,
		sindex, songInfo, width);
	...
	SwingUtilities.invokeLater(new Runnable() {
		...
		if (data instanceof EventIntent) {
		    ...
		    if (messageIntent.getAction().equals(MessageIntent.UPDATE_LISTVIEW_ALPHA)) {
	...
	if (sindex != -1 && sindex < listViewItemComPanel.getComponentCount()) {
		...
		if (songMessage.getType() == SongMessage.INITMUSIC
		    ...
		    if (mSongInfo != null) {
			lstViewItemComItemPanel.getSongProgress()
				.setText(MediaUtils.formatTime((int) mSongInfo.getPlayProgress()) + "/"
	...
	if (sIndex != -1 && sIndex < listViewItemComPanel.getComponentCount()) {
	    ...
	    SongInfoDialog songInfoDialog = SongInfoTipManage.getSongInfoTipManage(55 * 5, 25 * 3).getSongInfoDialog();
	    songInfoDialog.updateUI(songInfo);

	    //
	    Point thisPoint = mainPanel.getPlayListPanel().getLocationOnScreen();

	    // 当前控件的位置
	    Point componentPoint = listViewItemComItemPanel.getLocationOnScreen();
     ...
     * @param eventIntent
     */
    private void refreshListViewItemComItemSingleClickPanelUI(EventIntent eventIntent) {
	// 刷新新的面板ui
	int newPSingleClickIndex = eventIntent.getpIndex();
	if (newPSingleClickIndex == -1 || newPSingleClickIndex >= listViewPanel.getComponentCount()) {
	    return;
	}
	...
	if (oldsSingleClickIndex == newsSingleClickIndex && oldPSingleClickIndex == newPSingleClickIndex)
	    return;

	ListViewItemPanel newItemPanel = (ListViewItemPanel) listViewPanel.getComponent(newPSingleClickIndex);

	ListViewItemComPanel newlistViewItemComPanel = (ListViewItemComPanel) newItemPanel.getComponent(1);
	if (newsSingleClickIndex != -1 && newsSingleClickIndex < newlistViewItemComPanel.getComponentCount()) {
	...
	// 刷新旧的面板
	if (oldPSingleClickIndex == -1 || oldPSingleClickIndex >= listViewPanel.getComponentCount()) {
	    return;
	}

	ListViewItemPanel oldItemPanel = (ListViewItemPanel) listViewPanel.getComponent(oldPSingleClickIndex);

	ListViewItemComPanel oldlistViewItemComPanel = (ListViewItemComPanel) oldItemPanel.getComponent(1);
	if (oldsSingleClickIndex != -1 && oldsSingleClickIndex < oldlistViewItemComPanel.getComponentCount()) {
     ...
     * @param eventIntent
     */
    private void refreshListViewItemComItemDoubleClickPanelUI(EventIntent eventIntent) {
	// 刷新新的面板ui
	int newPDoubleClickIndex = eventIntent.getpIndex();
	if (newPDoubleClickIndex == -1 || newPDoubleClickIndex >= listViewPanel.getComponentCount()) {
	    return;
	}
	...
	if (oldsDoubleClickIndex == newsDoubleClickIndex && oldPDoubleClickIndex == newPDoubleClickIndex) {
	    return;
	}

	ListViewItemPanel newItemPanel = (ListViewItemPanel) listViewPanel.getComponent(newPDoubleClickIndex);

	ListViewItemComPanel newlistViewItemComPanel = (ListViewItemComPanel) newItemPanel.getComponent(1);
	if (newsDoubleClickIndex != -1 && newsDoubleClickIndex < newlistViewItemComPanel.getComponentCount()) {
	...
	// 刷新旧的面板
	if (oldPDoubleClickIndex == -1 || oldPDoubleClickIndex >= listViewPanel.getComponentCount()) {
	    return;
	}

	ListViewItemPanel oldItemPanel = (ListViewItemPanel) listViewPanel.getComponent(oldPDoubleClickIndex);

	ListViewItemComPanel oldlistViewItemComPanel = (ListViewItemComPanel) oldItemPanel.getComponent(1);
	if (oldsDoubleClickIndex != -1 && oldsDoubleClickIndex < oldlistViewItemComPanel.getComponentCount()) {
	...
	if (newShowPIndex != -1 && newShowPIndex < listViewPanel.getComponentCount()) {
	    //
	    ListViewItemPanel newItemPanel = (ListViewItemPanel) listViewPanel.getComponent(newShowPIndex);
	    ListViewItemHeadPanel newListViewItemHeadPanel = (ListViewItemHeadPanel) newItemPanel.getComponent(0);
	...
	if (oldShowPIndex != -1 && oldShowPIndex < listViewPanel.getComponentCount()
		&& oldShowPIndex != newShowPIndex) {
	    //
	    ListViewItemPanel oldItemPanel = (ListViewItemPanel) listViewPanel.getComponent(oldShowPIndex);
	    ListViewItemHeadPanel oldListViewItemHeadPanel = (ListViewItemHeadPanel) oldItemPanel.getComponent(0);
