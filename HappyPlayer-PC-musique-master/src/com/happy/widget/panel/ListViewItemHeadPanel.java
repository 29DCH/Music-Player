...
public class ListViewItemHeadPanel extends JPanel {
    ...
    public ListViewItemHeadPanel(JPanel mplayListPanel, JPanel mlistViewPanel, int width, String titleName, int size,
	    ListViewItemComPanel listViewItemComPanel, List<SongInfo> mSongInfo, int index) {
	this.pindex = index;
	...
	String leftIconPath = Constants.PATH_ICON + File.separator + "arrow_left.png";
	leftIcon = new ImageIcon(leftIconPath);
	leftIcon.setImage(leftIcon.getImage().getScaledInstance(height / 3, height / 3, Image.SCALE_SMOOTH));

	String downIconPath = Constants.PATH_ICON + File.separator + "arrow_down.png";
	downIcon = new ImageIcon(downIconPath);
	downIcon.setImage(downIcon.getImage().getScaledInstance(height / 3, height / 3, Image.SCALE_SMOOTH));

	statusLeftJLabel = new JLabel(leftIcon);
	statusLeftJLabel.setBounds(10, (height - height / 3) / 2, height / 3, height / 3);

	statusDownJLabel = new JLabel(downIcon);
	statusDownJLabel.setBounds(10, (height - height / 3) / 2, height / 3, height / 3);
	statusDownJLabel.setVisible(false);

	titleNameJLabel = new JLabel(titleName + "[" + size + "]");
	// titleNameJLabel.setForeground(new Color(71, 72, 73));
	titleNameJLabel.setBounds(statusLeftJLabel.getX() + statusLeftJLabel.getWidth() + 10, (height - height / 2) / 2,
		width / 2, height / 2);
	...
	menuButton = new BaseButton(menuButtonBaseIconPath, menuButtonOverIconPath, menuButtonPressedIconPath,
		height / 2, height / 2);
	menuButton.setToolTipText("列表菜单");
	menuButton.setBounds(width - height, (height - height / 2) / 2, height / 2, height / 2);
	...
	addSongMenu.addActionListener(new ActionListener() {
		...
		new Thread() {
			...
			int result = songchooser.showOpenDialog(ListViewItemHeadPanel.this);
			...
			if (result == JFileChooser.APPROVE_OPTION) {
				...
				if (!isSongExists(filePath, mSongInfo)) {
				    hasUpdate = true;
				    SongInfo songInfo = MediaUtils.getSongInfoByFile(filePath);
				    if (songInfo != null) {
					mSongInfo.add(songInfo);
					// 添加单首歌曲
					refreshListViewItemComPanelUI(pindex, mSongInfo.size() - 1,
						listViewItemComPanel, songInfo);

					titleNameJLabel.setText(titleName + "[" + mSongInfo.size() + "]");
				    }
				}
			    }
			    if (hasUpdate) {
				...
				MediaManage.getMediaManage().upDateSongListData(pindex, mSongInfo);
	...
	addSongFiledMenu.addActionListener(new ActionListener() {
		...
		new Thread() {
			...
			if (songfile.getSelectedFile() != null) {
			    String playlistname = JOptionPane.showInputDialog(ListViewItemHeadPanel.this, "请输入歌曲列表名",
				    "新建列表");
	...
	delPlaylistMenu.addActionListener(new ActionListener() {
		...
		new Thread() {
			...
			new Thread() {
			    ...
			    @Override
			    public void run() {
				int result = JOptionPane.showConfirmDialog(ListViewItemHeadPanel.this, "删除该播放列表?", "确认",
	...
	//
	// 列表标题面板
	ListViewItemHeadPanel listViewItemHeadPanel = new ListViewItemHeadPanel(playListPanel, listViewPanel, width,
		playlistname, size, listViewItemComPanel, songInfos, i);
     ...
     * @param files
     */
    private void updateListViewComPanelUI(final int pindex, final String playlistname, final File[] files) {
	new Thread() {
		...
		for (int i = 0; i < files.length; i++) {
		    File file = files[i];
		    if (!file.exists() || file.isDirectory() || !AudioFilter.acceptFilter(file))
			continue;
		    String filePath = file.getPath();
			...
			if (songInfo != null) {
			    songInfos.add(songInfo);
			    // 添加单首歌曲
			    refreshListViewItemComPanelUI(pindex, songInfos.size() - 1, listViewItemComPanel, songInfo);
			    listViewItemHeadPanel.getTitleNameJLabel()
				    .setText(playlistname + "[" + songInfos.size() + "]");
			}
		    }
		}
		if (hasUpdate) {
		    playListPanel.updateUI();
		    MediaManage.getMediaManage().upDateSongListData(pindex, songInfos);
		}
	    }
