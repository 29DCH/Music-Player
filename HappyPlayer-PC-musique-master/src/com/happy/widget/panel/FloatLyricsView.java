...
public class FloatLyricsView extends JPanel implements Observer {
    ...
    public FloatLyricsView(int width, int height, MouseInputListener desLrcDialogMouseListener) {
	...
	paintHLDEF = new GradientPaint(x, y - height / 2, Constants.DESLRCNOREADCOLORFRIST[Constants.desktopLrcIndex],
		x, y + height, Constants.DESLRCNOREADCOLORSECOND[Constants.desktopLrcIndex], true);
    }
	...
	paintHLED = new GradientPaint(x, y - height / 2, Constants.DESLRCREADEDCOLORFRIST[Constants.desktopLrcIndex], x,
		y + height, Constants.DESLRCREADEDCOLORSECOND[Constants.desktopLrcIndex], true);
    }
	...
	SCALEIZEWORDDEF = fontSizeScale * maxSizeWord / Constants.desktopLrcFontMaxSize;

	INTERVAL = (float) (height - SCALEIZEWORDDEF * 2) / 3;

	baseFont = FontsUtil.getFontByFile(fontFilePath, Font.BOLD, (int) SCALEIZEWORDDEF);
    }
	...
	int clipY = (int) (heightY - textHeight + (fm.getDescent() + fm.getLeading()));
	...
	if (lyricsLineNum == -1) {
		...
		initPaintHLDEFColor(textRightX, (SCALEIZEWORDDEF + INTERVAL) * 2, textHeight);
		g2d.setPaint(paintHLDEF);
		...
		if (lyricsLineNum + 1 != lyricsLineTreeMap.size()) {
		    ...
		    int textHeight = fm.getHeight();
		    initPaintHLDEFColor(10, (SCALEIZEWORDDEF + INTERVAL), textHeight);
		    g2d.setPaint(paintHLDEF);
		    g2d.drawString(lyricsLeft, 10, SCALEIZEWORDDEF + INTERVAL);
		}
	    ...
	    g2d.setClip((int) textX, clipY, (int) lineLyricsHLWidth, currentTextHeight);
	...
	SwingUtilities.invokeLater(new Runnable() {
		...
		if (data instanceof MessageIntent) {
		    ...
		    if (messageIntent.getAction().equals(MessageIntent.DESKSCMANYLINELRCCOLOR)) {
			repaint();
		    } else if (messageIntent.getAction().equals(MessageIntent.DESKSCMANYLINEFONTSIZE)) {
			initSizeWord();
			showLrc(progress);
			repaint();
		    }
