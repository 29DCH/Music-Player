...
public class MainLrcPanel extends JPanel implements Observer {
    ...
    public MainLrcPanel(int width, int height, JPanel mainPanel, DesLrcDialog desktopLrcDialog) {
	...
	if (songInfo != null) {
		...
		KscUtil.loadKsc(mSongInfo.getSid(), mSongInfo.getTitle(), mSongInfo.getSinger(),
			mSongInfo.getDisplayName(), mSongInfo.getKscUrl(), SongMessage.KSCTYPELRC);
		...
		if (desktopLrcDialog.getFloatLyricsView().getHasKsc()) {
		    desktopLrcDialog.getFloatLyricsView().showLrc((int) mSongInfo.getPlayProgress());
		}
     ...
     * @param isFile
     */
    private void initKscLrc(final String sid, final String kscFilePath, final long duration, final boolean isFile) {
	new SwingWorker<Void, Void>() {
	    @Override
	    protected Void doInBackground() {
		if (isFile)
		    kscLyricsParser = KscLyricsManage.getKscLyricsParser(sid, kscFilePath);
		else
		    kscLyricsParser = KscLyricsManage.getKscLyricsParserByKscInputStream(sid);
