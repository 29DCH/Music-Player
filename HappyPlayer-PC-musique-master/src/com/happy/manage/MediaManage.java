...
public class MediaManage implements Observer {
	...
	if (mCategorys == null) {
	    ...
	    // 加载默认歌曲
	    String defFilePath = Constants.PATH_AUDIO + File.separator + "蔡健雅 - Beautiful Love.mp3";
	...
	SwingUtilities.invokeLater(new Runnable() {
		...
		if (data instanceof SongMessage) {
			...
			if (songInfo != null && songMessage.getSongInfo() != null) {
			    // 相同，则更新进度
			    if (songInfo.getSid().equals(songMessage.getSongInfo().getSid())) {
	...
	// 0是顺序播放 1是随机播放 2是循环播放 3是单曲播放 4单曲循环
