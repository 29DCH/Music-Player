...
public class MediaPlayerService implements Observer {
	...
	if (mediaPlayer != null) {
	    mediaPlayer.getAudioOutput().setVolume((float) (Constants.volumeSize * 1.0 / 100));
	}
    }
	...
	try {
		...
		if (millis != 0) {
		    isSeekFinish = false;
		    long seekBytes = AudioMath.millisToSamples(millis, track.getTrackData().getSampleRate());
		    mediaPlayer.seek(seekBytes);
		} else {
		    // 设置基本的进度
		    baseCurrentMillis = 0;
	...
	public void run() {
		...
		try {
		    Thread.sleep(100);
		    if (mediaPlayer != null && mediaPlayer.isPlaying() && isSeekFinish) {
			...
			if (songInfo != null && status == MediaManage.PLAYING) {
			    // 由于直接获取当前的进度，在绘画歌词的时候会比较闪烁，所以，这里采用获取line当前的frame所在的位置，再加上上一次快进的位置，得到当前的位置
			    // 由于这种方法，导致了在快进的时候，要重新创建一个播放器和记录快进后的播放进度
			    double currentMS = mediaPlayer.getAudioOutput().getCurrentMillis();
			    long progress = Math.round(currentMS);
			    // System.out.println(progress);
			    // 等于0时不刷新，防止快进时闪屏
			    if (progress == 0)
				continue;
			    songInfo.setPlayProgress(progress + baseCurrentMillis);
			    SongMessage msg = new SongMessage();
			    msg.setSongInfo(songInfo);
