...
public class TrayDialog extends JDialog implements Observer {
	...
	this.addFocusListener(new FocusListener() {
		...
		if (!isEnter) {
		    ObserverManage.getObserver().deleteObserver(TrayDialog.this);
		    dispose();
		}
	    }
	...
	// 退出面板
	TrayExitPanel trayExitPanel = new TrayExitPanel(cWidth, cHeight, trayEvent);
	trayExitPanel.setBounds(padding, height - padding - cHeight, cWidth, cHeight);

	// 分隔线面板
	JPanel separatorJPanel = new JPanel();
	separatorJPanel.setBackground(new Color(207, 207, 207));
	separatorJPanel.setBounds(padding * 2, trayExitPanel.getY() - 1 - padding * 2, cWidth - padding * 2, 1);

	// 锁面板
	trayLockPanel = new TrayLockPanel(cWidth, cHeight, trayEvent);
	trayLockPanel.setBounds(padding, separatorJPanel.getY() - padding * 2 - cHeight, cWidth, cHeight);

	// 歌词面板
	TrayLrcPanel trayLrcPanel = new TrayLrcPanel(cWidth, cHeight, trayEvent, dialogMouseListener, this);
	trayLrcPanel.setBounds(padding, trayLockPanel.getY() - padding - cHeight, cWidth, cHeight);

	JPanel separatorJPanel2 = new JPanel();
	separatorJPanel2.setBackground(new Color(207, 207, 207));
	separatorJPanel2.setBounds(padding * 2, trayLrcPanel.getY() - 1 - padding * 2, cWidth - padding * 2, 1);
	...
	songTitleLabel.setBounds(padding, separatorJPanel2.getY() - padding * 2 - cHeight, cWidth, cHeight);

	JPanel oPanel = new JPanel();
	oPanel.setOpaque(false);
	oPanel.setBounds(padding, songTitleLabel.getY() - padding * 2 - cHeight, cWidth, cHeight);
	// 初始化操作面板控件
	...
	playButton = new BaseButton(playButtonBaseIconPath, playButtonOverIconPath, playButtonPressedIconPath, bSize,
		bSize);
	...
	BaseButton preButton = new BaseButton(preButtonBaseIconPath, preButtonOverIconPath, preButtonPressedIconPath,
	...
	BaseButton nextButton = new BaseButton(nextButtonBaseIconPath, nextButtonOverIconPath,
		nextButtonPressedIconPath, (int) (bSize * 1.5), (int) (bSize * 1.5));
