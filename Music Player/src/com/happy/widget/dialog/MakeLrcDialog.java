...
public class MakeLrcDialog extends JDialog {
	...
	closeButton.setBounds(width - buttonSize - rightPad, buttonY, buttonSize, buttonSize);
	...
	comPanel.setBounds(1, titleHeight, width - 1 * 2, height - titleHeight - 1);
	// 高度
	int lH = titleHeight + 10;
	// 步骤图片
	stepJLabel = new JLabel();
	stepJLabel.setBounds(rightPad * 2, rightPad / 2, width - rightPad * 4, lH);
	...
	cardPanel.setBounds(rightPad, stepJLabel.getY() + stepJLabel.getHeight() + rightPad / 2, wC, hC);

	// 底部面板
	JPanel bottomPanel = new JPanel();
	bottomPanel.setBounds(rightPad, cardPanel.getY() + cardPanel.getHeight(), width - rightPad * 2, lH);
	...
	chanelButton.setBounds(bottomPanel.getWidth() - rightPad - bWSize, y, bWSize, bHSize);
	...
	nextButton.setBounds(chanelButton.getX() - rightPad - bWSize, y, bWSize, bHSize);
	nextButton.addActionListener(new ActionListener() {
		...
		if (cardIndex > 3)
		    cardIndex = 3;
		if (cardIndex == 2) {
		    String lrcComText = makeLrcLvRuPanel.getLrcComTextArea().getText();
		    if (lrcComText == null || lrcComText.equals("")) {
			cardIndex--;
			JOptionPane.showMessageDialog(null, "歌词内容不能为空", "提示", JOptionPane.WARNING_MESSAGE);
		    } else {
	...
	preButton.setBounds(nextButton.getX() - rightPad - bWSize, y, bWSize, bHSize);
	...
	finishButton.setBounds(chanelButton.getX() - rightPad - bWSize, y, bWSize, bHSize);
	finishButton.addActionListener(new ActionListener() {
		...
		File fp = new File(Constants.PATH_KSC + File.separator + fileName + ".ksc");
		try {

		    String encoding = "GB2312";// 设置文件的编码！！
		    OutputStreamWriter outstream = new OutputStreamWriter(new FileOutputStream(fp), encoding);
		    PrintWriter writer = new PrintWriter(outstream);
		    // writer.format(encoding, null);
		    writer.write(content);
		    writer.close();

		    JOptionPane.showMessageDialog(MakeLrcDialog.this, "歌词文件保存成功", "提示", JOptionPane.PLAIN_MESSAGE);
		    ...
		    e1.printStackTrace();
		    JOptionPane.showMessageDialog(MakeLrcDialog.this, "歌词文件保存失败", "提示", JOptionPane.ERROR_MESSAGE);
		}
	    }
	});
	//
	...
	String backgroundPath = Constants.PATH_ICON + File.separator + "step_" + i + ".png";
	ImageIcon background = new ImageIcon(backgroundPath);// 背景图片
	background.setImage(
		background.getImage().getScaledInstance(width - 10, stepJLabel.getHeight(), Image.SCALE_SMOOTH));
	...
	ImageIcon background = new ImageIcon(backgroundPath);// 背景图片
	background.setImage(background.getImage().getScaledInstance(width, height, Image.SCALE_SMOOTH));
	return background;
    }
