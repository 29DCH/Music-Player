...
public class DesLrcDialog extends JDialog implements Observer {
	...
	desOperatePanel = new DesOperatePanel(width, mHeight / 5, desLrcDialogMouseListener);
	desOperatePanel.setBounds((mWidth - width) / 2, 0, width, mHeight / 5);
	desOperatePanel.setVisible(false);

	floatLyricsView = new FloatLyricsView(mWidth, mHeight - mHeight / 5, desLrcDialogMouseListener);
	floatLyricsView.setBounds(0, desOperatePanel.getY() + desOperatePanel.getHeight(), mWidth,
		mHeight - mHeight / 5);
	...
	SwingUtilities.invokeLater(new Runnable() {
		...
		if (data instanceof MessageIntent) {
		    ...
		    if (messageIntent.getAction().equals(MessageIntent.LOCKDESLRC)) {
			initLock();
		    }
		}
	    }
