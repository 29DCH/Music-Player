...
public class DesOperatePanel extends JPanel {
    ...
    public DesOperatePanel(int width, int height, MouseInputListener desLrcDialogMouseListener) {
	...
	DesOperateButton iconButton = new DesOperateButton(iconButtonBaseIconPath, iconButtonOverIconPath,
		iconButtonPressedIconPath, (buttonSize - 5), (buttonSize - 5), desLrcDialogMouseListener, this);
	iconButton.setBounds(padding, (mHeight - (buttonSize - 5)) / 2 + 1, (buttonSize - 5), (buttonSize - 5));
	...
	preButton = new DesOperateButton(preButtonBaseIconPath, preButtonOverIconPath, preButtonPressedIconPath,
		buttonSize, buttonSize, desLrcDialogMouseListener, this);

	preButton.setBounds(iconButton.getX() + iconButton.getWidth() + padding, 0, buttonSize, buttonSize);
	...
	playButton = new DesOperateButton(playButtonBaseIconPath, playButtonOverIconPath, playButtonPressedIconPath,
		buttonSize, buttonSize, desLrcDialogMouseListener, this);

	playButton.setBounds(preButton.getX() + preButton.getWidth() + padding, 0, buttonSize, buttonSize);
	...
	pauseButton = new DesOperateButton(pauseButtonBaseIconPath, pauseButtonOverIconPath, pauseButtonPressedIconPath,
		buttonSize, buttonSize, desLrcDialogMouseListener, this);
	pauseButton.setBounds(preButton.getX() + preButton.getWidth() + padding, 0, buttonSize, buttonSize);
	...
	nextButton = new DesOperateButton(nextButtonBaseIconPath, nextButtonOverIconPath, nextButtonPressedIconPath,
		buttonSize, buttonSize, desLrcDialogMouseListener, this);

	nextButton.setBounds(pauseButton.getX() + pauseButton.getWidth() + padding, 0, buttonSize, buttonSize);
	...
	String increaseButtonPressedIconPath = iconPath + "desIncrease_down.png";

	// 增加按钮
	DesOperateButton increaseButton = new DesOperateButton(increaseButtonBaseIconPath, increaseButtonOverIconPath,
		increaseButtonPressedIconPath, buttonSize, buttonSize, desLrcDialogMouseListener, this);

	increaseButton.setBounds(nextButton.getX() + nextButton.getWidth() + padding, 0, buttonSize, buttonSize);
	...
	increaseButton.addActionListener(new ActionListener() {
		...
		new Thread() {
			...
			MessageIntent messageIntent = new MessageIntent();
			messageIntent.setAction(MessageIntent.DESKSCMANYLINEFONTSIZE);
			ObserverManage.getObserver().setMessage(messageIntent);
		    }
	...
	String decreaseButtonPressedIconPath = iconPath + "desDecrease_down.png";
	// 减少按钮
	DesOperateButton decreaseButton = new DesOperateButton(decreaseButtonBaseIconPath, decreaseButtonOverIconPath,
		decreaseButtonPressedIconPath, buttonSize, buttonSize, desLrcDialogMouseListener, this);

	decreaseButton.setBounds(increaseButton.getX() + increaseButton.getWidth() + padding, 0, buttonSize,
		buttonSize);
	...
	for (int i = 0; i < lrcColorPanels.length; i++) {
	    lrcColorPanels[i] = new DesLrcColorParentPanel(buttonSize, buttonSize, Constants.DESLRCNOREADCOLORFRIST[i],
	...
	// 制作歌词按钮
	DesOperateButton makeLrcButton = new DesOperateButton(makeLrcButtonBaseIconPath, makeLrcButtonOverIconPath,
		makeLrcButtonPressedIconPath, buttonSize * 2, buttonSize, mouseListener, this, true);
	...
	makeLrcButton.addActionListener(new ActionListener() {
		...
		if (Constants.playInfoID.equals("")) {
		    JOptionPane.showMessageDialog(null, "请选择歌曲", "提示", JOptionPane.ERROR_MESSAGE);
		} else {
		    MessageIntent messageIntent = new MessageIntent();
	...
	DesOperateButton lockButton = new DesOperateButton(lockButtonBaseIconPath, lockButtonOverIconPath,
		lockButtonPressedIconPath, buttonSize, buttonSize, desLrcDialogMouseListener, this);
	lockButton.setBounds(makeLrcButton.getX() + makeLrcButton.getWidth() + padding, 0, buttonSize, buttonSize);
	...
	DesOperateButton closeButton = new DesOperateButton(closeButtonBaseIconPath, closeButtonOverIconPath,
		closeButtonPressedIconPath, buttonSize, buttonSize, desLrcDialogMouseListener, this);
	closeButton.setBounds(lockButton.getX() + lockButton.getWidth() + padding, 0, buttonSize, buttonSize);
	...
	DesOperateLabel bgl = new DesOperateLabel(mWidth, mHeight, desLrcDialogMouseListener, this);
