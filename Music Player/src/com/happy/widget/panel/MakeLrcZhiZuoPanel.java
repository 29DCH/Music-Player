...
public class MakeLrcZhiZuoPanel extends JPanel implements Observer {
	...
	int oH = bHSize * 3;
	// 操作面板背景
	String obgBackgroundPath = Constants.PATH_ICON + File.separator + "zhizuo_op.png";
	ImageIcon obgBackground = new ImageIcon(obgBackgroundPath);
	obgBackground.setImage(obgBackground.getImage().getScaledInstance(width - 10, oH - 10, Image.SCALE_SMOOTH));
	...
	songLabel.setBounds(width - 120 + padding / 2, oH + padding / 2, 120, bHSize);
	songLabel.setText(MediaUtils.formatTime(0) + "/" + MediaUtils.formatTime(0));
	//
	...
	songSlider.addChangeListener(new ChangeListener() {
		...
		if (isStartTrackingTouch) {
		    songLabel.setText(MediaUtils.formatTime(((JSlider) e.getSource()).getValue()) + "/"
	...
	jScrollPane.setBounds(padding, songSlider.getY() + songSlider.getHeight() + padding, width - padding * 2, jh);
	...
	if (lyricsLineNum < lrcLists.size()) {
	    // 设置旧索引的界面ui
	    LrcListViewItemPanel lrcListViewItemPanel = (LrcListViewItemPanel) lrcListView.getComponent(lyricsLineNum);
	...
	LrcListViewItemPanel newLrcListViewItemPanel = (LrcListViewItemPanel) lrcListView.getComponent(lyricsLineNum);
	...
	SongInfo songInfo = MediaPlayerService.getMediaPlayerService().getSongInfo();
	...
	for (int i = 0; i < lrcListView.getComponentCount(); i++) {
	    LrcListViewItemPanel lrcListViewItemPanel = (LrcListViewItemPanel) lrcListView.getComponent(i);
	    LrcComPanel lrcComPanel = lrcListViewItemPanel.getLrcComPanel();
	    KscLyricsLineInfo kscLyricsLineInfo = lrcComPanel.getKscLyricsLineInfo();
	    if (kscLyricsLineInfo != null) {
	...
	for (int i = 0; i < lrcLists.size(); i++) {
	    ...
	    LrcListViewItemPanel lrcListViewItemPanel = new LrcListViewItemPanel(i, kscLyricsLineInfo, this, comPanel,
		    width);
     ...
     * @return
     */
    private String getLineLyrics(String lrcComTxt, KscLyricsLineInfo kscLyricsLineInfo) {
	String newLrc = "";
	...
	if (mSongInfo != null) {
		...
		songLabel.setText(MediaUtils.formatTime(0) + "/" + MediaUtils.formatTime((int) mSongInfo.getDuration()));
		...
		if (!isStartTrackingTouch) {
		    ...
		    songLabel.setText(MediaUtils.formatTime((int) mSongInfo.getPlayProgress()) + "/"
	...
	DefaultKeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher(keyListener);
    }
	...
	DefaultKeyboardFocusManager.getCurrentKeyboardFocusManager().removeKeyEventDispatcher(keyListener);
    }
