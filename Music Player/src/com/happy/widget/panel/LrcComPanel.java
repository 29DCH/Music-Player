...
public class LrcComPanel extends JPanel {
    ...
    private KscLyricsLineInfo kscLyricsLineInfo;
    private int width;
    private int height;
    /**
     * 面板鼠标事件
    ...
    public LrcComPanel(int lyricsLineNum, JPanel comPanel, MakeLrcZhiZuoPanel makeLrcZhiZuoPanel,
	    KscLyricsLineInfo kscLyricsLineInfo, int lWidth, int bHeight) {
	this.lyricsLineNum = lyricsLineNum;
	this.comPanel = comPanel;
	this.kscLyricsLineInfo = kscLyricsLineInfo;
	this.makeLrcZhiZuoPanel = makeLrcZhiZuoPanel;
	this.width = lWidth;
	this.height = bHeight;

	lrcComTexts = kscLyricsLineInfo.getLyricsWords();
	...
	baseFont = FontsUtil.getFontByFile(fontFilePath, Font.BOLD, (int) height / 2);
    }
	...
	// 减去fm.getDescent() + fm.getLeading() 绘画时才能把全文字绘画完整
     ...
     * @return
     */
    public KscLyricsLineInfo getKscLyricsLineInfo() {
	if (isComFinish) {
		...
		int time = wordDisInterval.getEndTime() - wordDisInterval.getStartTime();
		wDisIntervals[j] = time;
	    }
	    kscLyricsLineInfo.setStartTime(startTime);
	    kscLyricsLineInfo.setStartTimeStr(startTimeStr);
	    kscLyricsLineInfo.setEndTime(endTime);
	    kscLyricsLineInfo.setEndTimeStr(endTimeStr);
	    kscLyricsLineInfo.setWordsDisInterval(wDisIntervals);
	    return kscLyricsLineInfo;
	}
	return null;
    }
