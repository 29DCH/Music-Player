...
public class KscManyLineLyricsView extends JPanel implements Observer {
	...
	for (int i = lyricsLineNum - 1; i >= 0; i--) {
		...
		float textSize = SIZEWORDHL - (SIZEWORDHL - SCALEIZEWORDDEF) * mCurFraction;
	    ...
	    g2d.drawString(text, textX, offsetY + (SCALEIZEWORDDEF + INTERVAL) * i);
	...
	for (int i = lyricsLineNum + 1; i < lyricsLineTreeMap.size(); i++) {
	    if (offsetY + (SCALEIZEWORDDEF + INTERVAL) * i > height - (SCALEIZEWORDDEF + SCALEIZEWORDDEF / 2)) {
		break;
	    }
	...
	if (lyricsLineNum != -1) {
	    ...
	    // 因为有缩放效果，有需要动态设置歌词的字体大小
	    float textSize = SCALEIZEWORDDEF + (SIZEWORDHL - SCALEIZEWORDDEF) * mCurFraction;
		...
		Rectangle2D rcBeforeWord = fm.getStringBounds(lyricsBeforeWord, g2d);
		// 当前歌词之前的歌词长度
	    ...
	    // 画当前歌词
	    g2d.drawString(lineLyrics, textX, offsetY + (SCALEIZEWORDDEF + INTERVAL) * lyricsLineNum);
	    ...
	    // 这里不知为何还要减去fm.getDescent() + fm.getLeading() 绘画时才能把全文字绘画完整
	    int clipY = (int) (offsetY + (SCALEIZEWORDDEF + INTERVAL) * lyricsLineNum - height
	...
	public void mousePressed(MouseEvent e) {
		...
		String imagePath = Constants.PATH_ICON + File.separator + "cursor_drag.png";
		Image image = new ImageIcon(imagePath).getImage();
		draggedCursor = tk.createCustomCursor(image, new Point(10, 10), "norm");
	    }
	    // 设置鼠标的图标
	...
	public void mouseReleased(MouseEvent e) {
		...
		String imagePath = Constants.PATH_ICON + File.separator + "cursor_pressed.png";
		Image image = new ImageIcon(imagePath).getImage();
		pressedCursor = tk.createCustomCursor(image, new Point(10, 10), "norm");
	    }
	    // 设置鼠标的图标
	...
	public void mouseDragged(MouseEvent e) {
	    if (draggedCursor == null) {
		String imagePath = Constants.PATH_ICON + File.separator + "cursor_drag.ico";
		Image image = new ImageIcon(imagePath).getImage();
		draggedCursor = tk.createCustomCursor(image, new Point(10, 10), "norm");
	    }
	    // 设置鼠标的图标
	...
	SwingUtilities.invokeLater(new Runnable() {
		...
		if (data instanceof MessageIntent) {
		    ...
		    if (messageIntent.getAction().equals(MessageIntent.KSCMANYLINELRCCOLOR)) {
			initColor();
			repaint();
		    } else if (messageIntent.getAction().equals(MessageIntent.KSCMANYLINEFONTSIZE)) {
			//
			initSizeWord();
			showLrc(progress);
			repaint();
	...
	if (newLyricsLineNum == -1) {
	    ...
	    if (lyricsLineNum != newLyricsLineNum || oldFontSizeScale != fontSizeScale) {
		lyricsLineNum = newLyricsLineNum;
		oldOffsetY = height / 2 - (SCALEIZEWORDDEF + INTERVAL) * lyricsLineNum + sy;
	    }
